...
router = express.Router({ mergeParams: true }),
      // create the subrouter with an option to allow access to _all_ the URL parameters (important for `req.param.base`)
responseHelper = require('./redis-response-helpers.module.node.js'); // helpers that cover handling responses and errors from Redis

let client;
function setClient(inClient) {
  // `client` into this scope
...
function (req, res, next) {
  // request, response and next middleware
